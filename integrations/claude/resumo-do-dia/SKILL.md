...
argument-hint: "[escopo em texto livre, ex. 'coisas da verboo']"
---
...
1. Descubra o escopo. Chame a ferramenta `resolve_scope` do MCP `daily_digest` com:
   ...
   quer, depois PARE (não colete nada ainda). Guarde `workspace` e `project`.
...
5. Leia `outputs` no retorno:
   - `outputs.markdown == true`: chame `write_digest` (markdown, date="today", mesmo workspace)
     e informe o caminho.
   - `outputs.whatsapp == true`: se houver ferramenta de WhatsApp nesta sessão
     (whatsmiau/Composio), use o `whatsapp_target` retornado, monte uma versão curta e, se
